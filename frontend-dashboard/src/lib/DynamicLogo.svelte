<script lang="ts">
    ...
    export let powerTypes: PowerTypes | undefined;

    // Start with the background grey as a backup
    let logoColor = [51,51,51];
    $: {
        if (powerTypes) {
            logoColor = calculateLightColour(powerTypes);
        }
    }
    let currentWidth = 0;
</script>
<div bind:clientWidth={currentWidth} style="--current-width: {currentWidth}px;">
    <svg viewBox="0 0 1000 1000" fill="currentColor" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" style={`--color: ${logoColor.join(", ")}`} class:loaded={!!powerTypes}>
