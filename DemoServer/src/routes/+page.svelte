<script lang="ts">
  ...
  let fakeData = derived(sliderValue, (value) =>
    generateFakeData(100 - value[0])
  );
  ...
  function calculatePercentRenewable(data: PowerTypesResponse) {
    ...
    const bad_generation_mw =
      power_types.coal.generation_mw +
      power_types.gas.generation_mw +
      power_types.diesel.generation_mw +
      power_types.co_gen.generation_mw;
  ...
  function pipFormatter(
    ...
    return (
      Math.round(
        calculatePercentRenewable(generateFakeData(100 - value)) * 100
      ) + "%"
    );
  }
</script>
